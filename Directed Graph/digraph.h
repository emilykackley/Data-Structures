...
{
    private:
        unsigned int m_size; //This should be initialized to the value NOT_YET_VISITED
    ...
    public:
        ...
        // Purpose: destructor
        // Postconditions: removes all edges and deallocates all dynamically allocated
        //   memory associated with the data structure (via clear)
        ~DirectedGraph() {clear();}
        ...
        // Purpose: add edge(i,j) with weight weight
        // Parameters: i is source vertex, j is destination vertex, w is weight
        //   (note: default value for weight is 1)
        // Postconditions: edge (i,j) is added to the graph with weight weight
        // Exceptions: if i or j >= # vertices in this graph, then throw an out_of_range error
        ...
        // Exceptions: if i or j >= # vertices in this graph then trhow
        //   out_of_range error
        bool isEdge(const unsigned int i, const unsigned int j) const;
        
        // Purpose: get the weight associated with edge (i,j)
        // Parameters: i is source vertex, j is destination vertex
        // Returns: the weight associated with edge (i,j); if no such edge
        //   exists, return 0
        // Exceptions: if i or j>= # vertices in this graph, then throw and
        //   out_of_range error
        int getEdgeWeight(const unsigned int i, const unsigned int j) const;
        
        // Purpose:set the weight associated with edge (i,j)
        // Parameters: i is source vertex, j is destination vertex, w is the 
        //   value to be associated with edge (i,j)
        // Postconditions: the weight associated with edge (i,j) has been set
        //   to w
        // Exceptions: if i or j >= # vertices in this graph, then throw an 
        //   out_of_range error
        void setEdgeWeight(const unsigned int i, const unsigned int j, const int w);
        ...
        // Exceptions: if i>= # vertices in this graph, then throw an out_of_range error 
        ...
        // Postconditions: the data value associated with vertex i has been set to data
        ...
        // Returns: the sum of the inDegree and outDegree of every vertex in this graph 
        ...
        // Purpose: determine whether this graph is complete (i.e., is simple 
        //   and there is an edge (u,v) or (v,u) for every pair of vertices u and v)
        ...
        //   should begin; visited is a 1D array of size N (essentially passed by reference)
        //   since it is an array); upont termination of this function, visited [i] 
        //  should be an integer indicating when vertex i was visited (e.g.,
        //  visted[start] = 0, if there is an edge (start,u) then visited[u]=1 
        //  etc); if a vertex v is never visited in a bfs beginning at start, then
        //  then visited[v] should be set to NOT_YET_VISITED
        // Exceptions: if start >= # vertices in this graph, then throw
        //   an out_of_range error 
        void bfs(const unsigned int start, int visited[]) const;
        
        // Purpose: perform a depth-first search (DFS) on this grpah starting at 
        //   specified vertex 
        // Parameters: start is the vertex from which the depth-first search should
        //   should begin; visited is a 1D array of size N (essentially passed by reference 
        //   since it is an array); upon termination of this function, visited[i] should 
        //   an integer indicating when vertex i was visited (e.g., visited[start] = 0;
        //   if there is an edge (start,u) then visited[u] =1; 
        //   if there is an edge (u,v) then visited[v]=2, etc.); if a vertex 
        //   v is never visited in a dfs beginning at start, then visited[v]
        //   should be set to NOT_YET_VISITED
        // Exceptions: if start >= # vertices in this graph, then throw 
        //   an out_of_range error 
        void dfs(const unsigned int start, int visited[]) const;
        
        // Purpose: perform a recursive depth-first search on this graph 
        //   starting at the specified vertex passed
        // Parameters: start is the vertex from which the DFS should begin upon 
        //   termination of this funtion, visited[i] should be an int indicating when vertex was visited. 
        //   Default for visited should be -1. Cont is an int that keeps track of the depth 
        // Exceptions: if start >= #vertices in graph throw out_of_range error
        void recursiveDfs(unsigned int start, int visited[], int count) const;
};
#include "digraph.hpp"
