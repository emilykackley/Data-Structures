/* File: Vector.h
*  Purpose: Definition of a templated Vector Class
*/
...
class Vector
{
	private:
		...
		unsigned int m_max_size; //current maximum size of vector data
		T* m_data; //data
	public:
		...
		//Postcondtions: if current size + 1 == maximum size BEFORE the insertion
		//	of x, memory for data is doubled; x now at the formerly "leftmost available"
		//	position in the vector, and current size incremented by 1
		void push_back(const T& x);

		//Purpose: removes one instance of the value x from the vector if it exists
		//Parameters: x is the value to be removed from teh vector
		//Postconditions: if x is in the vector, m_size is decremented and the first
		//	instance of x is removed from the vector. If x is not in the vector,
		//	do nothing
		//	If m_size <= m_max_size/4, half the amount of memory available
		...
		//Postconditions: current size, maximum size,a nd data set to those of rhs
